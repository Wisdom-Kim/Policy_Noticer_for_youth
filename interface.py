...
from custom_filter import Filter  # custom_filter 모듈에서 Filter 클래스 가져오기
...
class UI:
    ...
    def create_widgets(self,my_filter):
        # 카테고리 라벨 및 체크박스
        ...
        for i, category in enumerate(my_filter.cate_list()):
            var = tk.BooleanVar()
        ...
        for i, target in enumerate(my_filter.target_list()):
            var = tk.BooleanVar()
    ...
    def apply_filter(self,my_filter):
        #선택한 요소의 인덱스를 필터 객체에 저장
        my_filter.cate = [idx for idx, var in enumerate(self.category_vars) if var.get()]
        my_filter.target = [idx for idx, var in enumerate(self.target_vars) if var.get()]
        self.root.destroy()
